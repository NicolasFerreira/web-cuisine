...
<form method="POST" enctype="multipart/form-data" action="{{ action('SdbController@update', [$sdb->id]) }}" class="form-add">
	@csrf
	<h3>Modification d'une salle de bain dans votre catalogue</h3>
	<div>
		<label for="titre">Titre : </label>
		<input type="text" name="titre" required placeholder="Titre" id="titre" value="{{ $sdb->titre }}">
	</div>
	<div>
		<label for="prix">Prix : </label>
		<input type="text" name="prix" required placeholder="A partir de xxx €" id="prix" value="{{ $sdb->prix }}">
	</div>
	<div>
