...
<form method="POST" enctype="multipart/form-data" action="{{ action('PartenairesController@store') }}" class="form-add">
	@csrf
	<h3>Gestion des partenaires</h3>

    <div>
        <label for="link">Lien ( url ) : </label>
        <input type="text" name="link" required placeholder="Lien site internet" id="link">
    </div>
